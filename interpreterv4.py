...
class Interpreter(InterpreterBase):
    ...
    NIL = BrewinNil()
    # TODO: This is probably not good code, but it ensure we have a single NIL object always (can use is)
    BrewinObject.NIL = NIL
                    ...
                    if self.vars.get(func_name) is not None:
                        ...
                        if self.vars.get(func_name).get(func_num_args) is not None:
                            super().error(ErrorType.NAME_ERROR,
                                    "Two functions declared with the same name and argument count.",
                            )
                        else:
                            # Add function
    ...
    def __do_function(self, func_elem, args_expr, closure = None):
        ...
        if (func_elem.elem_type != "func" and func_elem.elem_type != "lambda") or "statements" not in func_elem.dict or "args" not in func_elem.dict:
            print("ERROR: Running __do_function on invalid function element!")
        # Trace output
        if self.trace_output:
    ...
    def __do_statement(self, statement_elem):
        ...
        if statement_elem.elem_type == "=":
            ...
            if "expression" not in statement_elem.dict or "name" not in statement_elem.dict:
                print("ERROR: Statement element has no expression or name! ")
            # Trace output
            if self.trace_output:
                print(f'Statement is assignment with name {statement_elem.dict["name"]}.')
            # Assignment logic
            var_name = statement_elem.dict["name"]
            value = self.__get_expr(statement_elem.dict["expression"])
            # If we are referencing a variable's object
            if var_name.find(".") != -1:
                [obj_name, mem_name] = var_name.split(".")
                ...
                if not isinstance(obj, BrewinObject):
                    super().error(ErrorType.TYPE_ERROR,
                             f"Attempting to access member fields of non-object value"
                    )
                # If we are setting the proto field to a non object value
                if mem_name == "proto" and not isinstance(value, BrewinObject) and not isinstance(value, BrewinNil):
                    super().error(ErrorType.TYPE_ERROR,
                             f"Setting proto to a non-obj or non-nil value"
                )
                obj.set_member(mem_name, value)
            # If we are not referring to an object's member
            else:
        ...
        # Statment logic (fcall)
            ...
            if "args" not in statement_elem.dict or "name" not in statement_elem.dict:
                print("ERROR: Fcall element has no args or name!")
            # Trace output
            if self.trace_output:
            ...
            "else_statements" not in statement_elem.dict:
                print("ERROR: Statement if element has no condition or statements or else_statements!")
            # Trace output
            if self.trace_output:
            ...
            if "condition" not in statement_elem.dict or "statements" not in statement_elem.dict:
                print("ERROR: Statement while element has no condition or statements.")
            # Trace output
            if self.trace_output:
            ...
            if "expression" not in statement_elem.dict:
                print("ERROR: Statement return element has no expression.")
            ...
            if not isinstance(obj, BrewinObject):
                super().error(ErrorType.TYPE_ERROR,
                            f"Attempting to access member field of non-object value"
                )
            ...
            print(f"ERROR: Unknown statement type {statement_elem.elem_type}.")
    ...
    def __get_expr(self, expr_elem):
        ...
        OPS = { (int, int): INT_OPS,
                ...
                (BrewinObject, BrewinObject): OBJECT_OPS,
        }
        ...
        if expr_elem.elem_type in VAR:
            ...
            if "name" not in expr_elem.dict:
                print("ERROR: Variable expression has no name!")
            # Trace output
            if self.trace_output:
            ...
            if var_name.find(".") != -1:
                ...
                res = obj.get_member(mem_name)
                if res is None:
                    super().error(ErrorType.NAME_ERROR,
                             f"Object {obj_name} has no member {mem_name}"
                    )
            # Not referencing a variable through an object
            else:
        ...
        # Value type
            ...
            if self.trace_output:
                print(f'Evaluating {expr_elem.elem_type} value w val {expr_elem.dict.get("val", None)}.')
            # Value logica
        ...
        # Expr type
            ...
            if "op1" not in expr_elem.dict:
                print("ERROR: Expression expression has no op1!")
            # Get operands
            ...
            # Trace output
            if self.trace_output:
                print(f"Evaluating operator expression {expr_elem.elem_type} {op1} {op2}.")
            # Operator logic
        ...
        # Not var, value, expr, fcall, or @
        else:
            print("ERROR: expression type not VAR, VALUE, or EXPR!")
    ...
    def __do_fcall(self, fcall_elem):
        ...
        if fcall_elem.elem_type != "fcall" or "name" not in fcall_elem.dict or "args" not in fcall_elem.dict:
            print("ERROR: Running __do_fcall on invalid fcall element!")
        # Trace output
        if self.trace_output:
